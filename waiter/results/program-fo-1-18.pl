...
robot3(A,B):-robot2(A,C),turn_cup_over(C,B).
robot2(A,B):-turn_cup_over(A,C),robot1(C,B).
robot1(A,B):-pour_coffee(A,C),move_right(C,B).
robot3(A,B):-pour_tea(A,C),move_right(C,B).
robot(A,B):-wants_coffee(A),robot1(A,B).
